<?php
function val(String $strJson)
{
  return intval(json_decode($strJson)->val);
}
?>
...
<main class="d-flex justify-content-center align-items-center">
  <<?= ($role === 'E') ? 'form enctype="multipart/form-data"' : 'div' ?> id="update-record-form" class="d-flex flex-column m-1" method="post">
    <div class="doc-header d-flex justify-content-between align-items-center flex-wrap p-2 gap-2">
      ...
      <?php if ($role === 'E') { ?>
        <button class="btn btn-primary" type="button" id="merge-btn"><i class="fa-solid fa-object-ungroup"></i> Merge</button>
      <?php } ?>
      <?php if ($role === 'E') { ?>
        <button class="btn btn-primary" type="button" id="move-btn"><i class="fa-solid fa-arrows-left-right"></i> Move Shelf</button>
      <?php } ?>
      <?php if ($role === 'E') { ?>
        <button class="btn btn-outline-primary" type="button" id="delete-record-btn" onclick="VIEW_RECORD.onSubmit(this);"><i class="fa-solid fa-box-archive"></i> Archive</button>
      <?php } ?>
      <?php if ($role === 'E') { ?>
