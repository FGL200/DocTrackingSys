...
async function setupEncoded_monthly() { 
    ...
    function addSeries(dates, encoders) {
        ...
        // this is the function from stackoverflow
        ...
        const seriesEncoders = [];

        chart.series.each(series=>{
            seriesEncoders.push(series.get("name"));
        })


        console.log(seriesEncoders);
        for (let i = 0; i < encoders.length; i++) {
            if(seriesEncoders.indexOf(encoders[i])>=0) {
                console.log(seriesEncoders.indexOf(encoders[i]), " ", encoders[i]);
                continue;
            } else {
