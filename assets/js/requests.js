async function Load_RequestList() {
  ...
  const resp = (await Helper.api('request/all', "json", Helper.createFormData()))
    .map(v => ({ ...v, status: JSON.parse(v.status) }));
  console.log({ resp })
  ...
  resp.forEach((v, i) => tbody += `
    <tr style="width: 10px; !important">
      <td class="text-center text-${Helper.dateDiff(new Date().toDateString(), v.due_date) >= 0 ? 'danger' : 'black'} align-middle">
        ${CONST_ROLE != 'V' ? `${Helper.AsID(v.id, 6, "0")}` :
      `<div class="dropdown">
          <a  class="link dropdown-toggle text-${Helper.dateDiff(new Date().toDateString(), v.due_date) >= 0 ? 'danger' : 'black'}" 
              href="#" role="button" 
              id="dm-${i}" 
          ...
          <ul class="dropdown-menu" aria-labelledby="dm-${i}">
            <li>
              <a 
                class="dropdown-item text-success btn-open-released"
                data-binder-id="${v.id}"
              ...
              <a 
                class="dropdown-item text-danger btn-open-notReleased"
                data-binder-id="${v.id}"
                data-binder-lname="${v.lname}"
                data-binder-fname="${v.fname}"
                data-binder-mname="${v.mname}"
                href="#id=${Helper.AsID(v.id, 6, '0')}-notReleased"
              >
                <i class="fas fa-exclamation"></i> Not Released
              </a>
            </li>
            <hr class="m-1" />
            <li>
              <a 
                ...
                data-binder-due_date="${v.due_date}"
      ...
      <td class="align-middle text-${Helper.dateDiff(new Date().toDateString(), v.due_date) >= 0 ? 'danger' : 'black'}">${v.lname}, ${v.fname} ${v.mname}</td>
      <td class="align-middle text-${Helper.dateDiff(new Date().toDateString(), v.due_date) >= 0 ? 'danger' : 'black'}">${v.file}</td>
      <td class="align-middle text-${Helper.dateDiff(new Date().toDateString(), v.due_date) >= 0 ? 'danger' : 'black'}">${Helper.toDetailedDate(v.created_at)}</td>
      <td class="align-middle text-${Helper.dateDiff(new Date().toDateString(), v.due_date) >= 0 ? 'danger' : 'black'}">
        ${Helper.toDetailedDate(v.due_date)}
      </td>
      <td class="align-middle">
        <div class="alert border p-2 text-center m-0
          alert-${v.status.value == "Pending" ? 'warning' : v.status.value == "Released" ? 'success' : 'danger'} 
          border-${v.status.value == "Pending" ? 'warning' : v.status.value == "Released" ? 'success' : 'danger'}">
          ${v.status.value ?? 'No status'}
        </div>
      </td>
    </tr>
  `);
  ...
  Helper.DataTable_Init("#table-request", thead + tbody, () => {
    ...
    Helper.fm('.btn-open-notReleased', v => {
      Helper.clearEvent(v, 'click', OpenModal_NotReleased);
      Helper.on(v, 'click', OpenModal_NotReleased);
    });
...
async function OpenModal_NotReleased(e) {
  const data = {
  ...
  MODAL.setSize('md')
  MODAL.setTitle('<h4 class="m-3"><i class="fas fa-exclamation"></i> Not Released</h4>');
  MODAL.setBody(Helper.replaceLayout(await Helper.template('request/notReleased'), data));
  MODAL.setFooter(`
  ...
  MODAL.onSubmit(async (e, form_data) => {
    ...
    if (Helper.formValidator(form_data, ["reason"], v => v == '').length > 0) {
    ...
    const body = {
      id: Helper.getDataFromFormData(form_data).id,
      reason: Helper.getDataFromFormData(form_data).reason,
      status: 'Not Released',
    }
    console.log({ body });
    const status = (await Helper.api(`request/${body.id}/update`, 'json', Helper.createFormData({ status: body.status, reason: body.reason }))).status;
    if (status == "success") {
...
async function OpenModal_Released(e) {
  ...
  MODAL.onSubmit(async (e, form_data) => {
    ...
    console.log({ body });
    const status = (await Helper.api(`request/${body.id}/update`, 'json', Helper.createFormData({ status: body.status }))).status;
    if (status == "success") {
      MAIN.addNotif("Success", "Request is updated.", "g");
      MODAL.close();
      await Load_RequestList();
...
async function OpenModal_Edit() {
  const data = {
    ...
    due_date: Helper.getDataBind(this, 'due_date'),
  }
  console.log({ data })
  MODAL.setSize('lg')
