...
async function Load_Table() {
  const resp = (await Helper.api('shelf/archives', 'json'));
  const thead = `
    <thead>
      <tr>
  ...
  Helper.DataTable_Init("#table_content", thead + tbody, () => {
    Helper.fm(".restore", v => Helper.on(v, "click", async function (e) {
      ...
      const resp = (await Helper.api(`shelf/${data.id}/update`, 'json', Helper.createFormData({ name: data.name, deleted_flag: 0 })));
      console.log({ resp })
      if (resp.status == 1) {
